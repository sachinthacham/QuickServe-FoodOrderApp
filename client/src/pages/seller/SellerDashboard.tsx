...
import { TrendingUp, Store, DollarSign, Package, Clock, CheckCircle, ChevronRight, Activity, Eye, LayoutDashboard, Plus, ArrowRight } from "lucide-react";
...
function SellerDashboardContent() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 pt-24 pb-12 relative overflow-hidden">
      ...
      <div className="absolute bottom-0 left-0 w-[400px] h-[400px] bg-orange-500/10 dark:bg-orange-500/5 rounded-full blur-[100px] -ml-24 -mb-24 pointer-events-none"></div>
      ...
      <div className="max-w-[1400px] mx-auto px-6 relative z-10">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12 animate-fade-in-down">
          <div>
            <div className="flex items-center gap-3 mb-2">
              ...
              <p className="text-orange-500 font-bold tracking-wider uppercase text-sm">Partner Restarauteur Area</p>
            </div>
            <h1 className="text-4xl md:text-5xl font-extrabold text-slate-900 dark:text-white tracking-tight">
              Seller Dashboard
            </h1>
            <p className="text-slate-500 dark:text-slate-400 mt-2 text-lg font-light max-w-2xl">
              Manage your restaurants, track incoming orders, and view your revenue stream securely.
            </p>
          </div>
          <div className="flex items-center gap-3">
            <Link
              to="/seller/restaurants"
              className="flex items-center gap-2 bg-slate-900 border border-slate-800 text-white dark:bg-white dark:border-white dark:text-slate-900 px-6 py-3 rounded-2xl shadow-sm hover:shadow-lg transition-all font-bold group"
            >
              <Plus className="w-5 h-5 group-hover:rotate-90 transition-transform" /> New Restaurant
            </Link>
          </div>
        </div>
        ...
        {error && (
          <div className="mb-8 p-4 glass-card border-red-500/20 bg-red-50/50 dark:bg-red-900/10 rounded-2xl flex items-start gap-4 text-red-600 animate-shake shadow-sm relative overflow-hidden">
            ...
            <LayoutDashboard className="w-6 h-6 shrink-0 mt-0.5" />
            <div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12 animate-fade-in-up" style={{ animationDelay: '0.1s' }}>
          ...
          <div className="glass-card bg-white dark:bg-slate-900 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-shadow group relative overflow-hidden">
            ...
            <div className="flex justify-between items-start relative z-10">
              <div>
                <p className="text-sm font-medium text-slate-500 dark:text-slate-400 mb-1">My Restaurants</p>
            ...
            <div className="mt-4 flex items-center text-sm font-medium text-green-500 gap-1 relative z-10">
              <TrendingUp className="w-4 h-4" /> <span>Active locations</span>
            </div>
          </div>
          ...
          <div className="glass-card bg-white dark:bg-slate-900 rounded-[2rem] p-6 border border-slate-100 dark:border-slate-800 shadow-sm hover:shadow-xl transition-shadow group relative overflow-hidden">
            ...
            <div className="mt-4 flex items-center text-sm font-medium text-yellow-500 gap-1 relative z-10">
              <span className="w-2 h-2 rounded-full bg-yellow-500 animate-pulse"></span> <span>Awaiting Preparation</span>
            </div>
          </div>
          ...
          <div className="glass-card bg-gradient-to-br from-slate-900 to-slate-800 dark:from-slate-800 dark:to-slate-900 rounded-[2rem] p-6 border border-slate-800 shadow-lg hover:shadow-xl transition-shadow group relative overflow-hidden">
            ...
            <div className="flex justify-between items-start relative z-10">
              <div>
                <p className="text-sm font-medium text-slate-400 mb-1">Confirmed Revenue</p>
             ...
             <div className="mt-4 flex items-center text-sm font-medium text-green-400 gap-1 relative z-10">
              <CheckCircle className="w-4 h-4" /> <span>Processed Successfully</span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-3 gap-8">
          ...
          <div className="xl:col-span-2 space-y-6 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
            <div className="flex items-center justify-between">
              <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
                <Activity className="w-6 h-6 text-orange-500" /> Incoming Orders
              </h2>
              <Link to="/seller/orders" className="text-sm font-semibold text-orange-500 hover:text-orange-600 flex items-center gap-1 group">
                Manage Orders <ChevronRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
              </Link>
            </div>
                    ...
                    <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                      {orders.slice(0, 5).map((order) => (
                        <tr key={order.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/20 transition-colors group">
                          ...
                          <td className="py-4 px-6 text-center">
                            <button className="text-slate-400 hover:text-orange-500 p-2 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors inline-block" title="View Order">
          ...
          <div className="space-y-6 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
            <h2 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
              <LayoutDashboard className="w-6 h-6 text-orange-500" /> Actions
            </h2>
            ...
            <div className="bg-white dark:bg-slate-900 rounded-[2rem] shadow-sm border border-slate-100 dark:border-slate-800 p-6 space-y-3">
              ...
              >
                ...
                <h4 className="font-bold text-slate-900 dark:text-white text-lg">My Restaurants</h4>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-light">Edit details, update menus, and change operating hours.</p>
              </Link>
              ...
              >
                ...
                <h4 className="font-bold text-slate-900 dark:text-white text-lg">Manage Orders</h4>
                <p className="text-sm text-slate-500 dark:text-slate-400 mt-1 font-light">Review, prepare, and dispatch customer orders.</p>
              </Link>
            </div>
          </div>
